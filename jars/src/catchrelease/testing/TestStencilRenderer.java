...
import catchrelease.entities.MoteEntityPlugin;
...
public class TestStencilRenderer implements LunaCampaignRenderingPlugin {
    ...
    public void spawnRandomMote() {
        ...
        SectorEntityToken mote = Global.getSector().getPlayerFleet().getContainingLocation().addCustomEntity(Misc.genUID(), "Mote", "catchrelease_Mote", null, new MoteEntityPlugin.Params(targetLoc, getRandomRarityColor()));
    ...
    public void render(CampaignEngineLayers layer, ViewportAPI viewport) {
        ...
        Vector2f loc = Global.getSector().getPlayerFleet().getLocation();
        StencilHelper.startDepthMask(stencil, SIZE, SIZE, loc, true);
        ...
        if (layer == CampaignEngineLayers.ABOVE) {
            for (SectorEntityToken mote : containingLoc.getEntitiesWithTag(MoteEntityPlugin.MOTE_TAG)) ((MoteEntityPlugin) mote.getCustomPlugin()).externalRender(viewport);
        }

        StencilHelper.endDepthMask();
    }
