...
| Resume the deferred Fisherman / Scientist quest discussion after the three fishing ships are finished | [NEEDLE_QUEST_CONCEPT.md](docs/NEEDLE_QUEST_CONCEPT.md) preserves the narrative decisions and open options. It does not govern current canon or authorize implementation; `LORE.md` remains the lore authority. |
...
- Keep all Java tools and automated checks under `jars/src`, in the normal module. Do not add separate test source trees or duplicate game classes. Tools may read existing game constants and types; do not change the in-game minigame classes to support them.
...
| `docs/NEEDLE_QUEST_CONCEPT.md` | Deferred quest brainstorming: the player's needle, Fisherman / Scientist conflict, narrative decisions and open options | Revisit after the three fishing ships are finished. Preserve the distinction between selected concepts and suggestions; this note does not amend `LORE.md`, describe implemented behavior or authorize implementation. |
