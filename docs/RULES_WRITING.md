...
```text
id: catchrelease_exBrief1        trigger: DialogOptionSelected   conditions: $option == catchrelease_exBrief1
text:    (first beat)
options: catchrelease_exBrief2:Continue
...
```text
id: catchrelease_exAsk           trigger: DialogOptionSelected   conditions: $option == catchrelease_exAsk
...
text:    (the answer)
script:  $catchrelease_exAskedPay = true
         FireAll CatchReleaseExQuestions
```
...
```text
id: catchrelease_exGreet             trigger: CatchReleaseExGreeting
text:    (general greeting)

id: catchrelease_exGreetOffered      trigger: CatchReleaseExGreeting   conditions: $catchrelease_ex_stage == OFFERED
text:    (greeting while the offer is open)

id: catchrelease_exGreetFailed       trigger: CatchReleaseExGreeting   conditions: $catchrelease_ex_stage == FAILED score:100
...
- Equal rows for random variety are deliberate; a note in the notes column tells the next writer so (a project convention; SotF leaves the column empty).
...
```text
$option = catchrelease_exShowOffer 0
...
| Opening a comm link with a fleet | The fleet encounter's embedded rules dialog makes the commander active and fires `FireBest OpenCommLink`; if nothing matches, the player sees static | An `OpenCommLink` row conditioned on the fleet flag its spawner set, such as `$entity.catchrelease_x` |
...
- **Quest state.** Ask the owner with a condition verb (`CatchReleaseCMD <query>`) or a mission reference instead of copying quest state into memory for rules to read.
- **Case.** Keys and trigger names are case sensitive. `$catchrelease_x` and `$catchrelease_X` are different keys; a `FireAll` on a misspelled trigger finds no rows and shows nothing. SotF ships this bug: two rows fire `sotfWendigoCHOffers` while the menu's rows use `sotfWendigoCHoffers`.
...
```text
$catchrelease_exAskedPay = true               saved on the current owner
$catchrelease_exPayStr = "12,000 credits" 0   display value, gone after the dialog
$catchrelease_exVisits++                      count up; stores a Float
unset $catchrelease_exOfferShown              remove now
AddCredits 12000                              vanilla grant with its own receipt
CatchReleaseCMD takeWork                      project command verb
FireAll CatchReleaseExQuestions               rebuild the menu
```
...
| Fact | Owner | Example |
|---|---|---|
| Quest progress, targets, timers, decisions, counters | The quest's Java owner (a mission, intel or manager), read through a command verb or a mission reference | `Call $catchrelease_jobRef <action>`; a hub mission's `$<missionId>_stage` |
| "Already asked", "already introduced" and other conversation flags | The speaker's own memory: an unscoped key while the person is active | `$catchrelease_exAskedPay` |
| What a fleet is for | The fleet's memory, set by its spawner | `$catchrelease_fleetQuest` |
...
| A value shown in text | A String with expiry `0`, written before the row that shows it | `$catchreleaseReward` |

- Name new keys `$catchrelease_<feature><Name>` in lowerCamel, as `$catchrelease_fleetQuestType`. Give a key one spelling everywhere.
...
Narration is plain. Speech is in quotation marks. Catch.Release does not color speech per speaker. Use color only for meaning, as [DIALOGUE.md](DIALOGUE.md#shared-text-presentation) defines: highlights, gains and losses, item names.
...
- Write scoped tokens when the key is not local: `$player.catchrelease_x`. An unqualified token takes local first; if two other scopes hold the same name, which one wins is not defined.
...
| Back to a parent menu inside the conversation | A custom option such as `catchrelease_exBack` whose handler fires the parent menu. Bind Escape with `SetShortcut catchrelease_exBack ESCAPE` when it is the screen's way out. |
...
Catch.Release has one rules command, `CatchReleaseCMD`, whose first argument selects the verb ([Project routing](RULES.md#project-routing)). When adding a verb:
...
- **One block per feature.** A `#` row naming the feature opens it (`# bar job: chef`) and blank rows end it, as in SotF and vanilla; rows whose id starts with `#` are skipped by the loader. Long quests get sub-headers along their stages, as SotF's `# THE HAUNTED` block does.
...
Update this guide when a structural convention changes or a new pattern is adopted. Engine facts belong in [RULES.md](RULES.md) and mechanism procedures in [RULES_AUTHORING.md](RULES_AUTHORING.md); link to them rather than restating them. Replace SotF examples with Catch.Release rows where one shows the same pattern. Follow [CLAUDE.md](../CLAUDE.md#documentation-upkeep) for ownership and commit requirements.
