...
| `rules.csv` Fisherman stability aside | After the player has asked why the Fisherman is in another system, the Barely Holding dialogue band exposes a one-time `Are you okay?` main-menu branch. The acknowledgement, mirror question, and knot follow-up all terminate in an explicit `catchrelease_fisherBack` route, while the campaign-persistent asked flags enforce that order and prevent either observation from repeating on another boat. |
