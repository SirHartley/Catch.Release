...
| File | What it does |
|---|---|
| `CampedSpotJob.java` | The shared job. Two conditions rather than one — clearing the camp is the work, and any fish whose catch record names that exact rupture is the receipt. Both the authored offer and the intel state this plainly: the fleet goes first, then any fish from that exact water is brought back as proof. Asks `CampedSpot.isGone` and nothing more specific, so it never has an opinion about how the player did it. Its poll sends a dedicated intel update when the camp leaves, removes the mission marker from a surviving retreating fleet, and shifts the objective to the still-marked rupture. The offer only chooses the rupture and terms; the physical camper and pond claim are created on acceptance so discarded bar-event rolls cannot leave fleets behind. Active older saves repair their named-species ask to the pond receipt |
