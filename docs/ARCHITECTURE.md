...
the renderer. `catchreleaseMinigameLineClickSound` defaults to the registered `click_trigger.ogg`
one-shot. `catchreleaseMinigameLineClickOnRelease` chooses its input edge: `false` plays on
LMB-down and `true` plays on LMB-up. `catchreleaseMinigameLineHeldLoop` and
