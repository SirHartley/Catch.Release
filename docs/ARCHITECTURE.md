...
| `QuestDuration.java` | Deadline rungs of 30/60/90/120/180 days or no limit. Picks the smallest rung covering working days plus a round trip at a per-light-year rate, either to a known target or to the nearest system where the asks are satisfiable; unsatisfiable asks get no limit. Also the satisfiability gate: `satisfiableWithin` reports whether every ask can be filled within a given reach, since species ranges move monthly and a rolled demand can point at water that no longer exists. |
