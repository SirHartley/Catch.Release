...
| `LootResultPanel.java` | The mirror card listing treasure recovered alongside the fish; its side-panel exhibit uses the closed treasure chest while the delayed tally waits. Item names wrap inside its default width and rows grow vertically instead of stretching the screen for long blueprint names. Its success hook fires once when the delayed tally begins, after the fish readout finishes |
