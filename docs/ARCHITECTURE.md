...
The framework creates the distress entity, vanilla-style breadcrumb intel, and rules trigger. Providers own all event and quest content and may replace the breadcrumb's descriptive copy or the default jump-point orbit anchor. The shared encounter and comm entries require the fleet-owned `$distressFramework` Boolean. The saved instance also verifies the exact interaction fleet and instance reference before firing a provider trigger. Catch.Release's provider trigger requires both the distress and fleet-quest flags before it enters the common offer menu. Catch.Release supplies the stranded fleet, dead-engine scavenger, Hegemony Follower, Diktat State Dinner, Tri-Tachyon Claim Assay, League Mandate, and pirate Parley Fish events through `FleetQuest`.

### Other registrations

- `data/world/factions/default_ranks.json` registers named mod roles, including the Fisherman-side
  contacts, Fight Night staff, and specialist fleet-quest speakers.
