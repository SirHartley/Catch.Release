...
The framework creates the distress entity, vanilla-style breadcrumb intel, and rules trigger. Providers own all event and quest content and may replace the breadcrumb's descriptive copy. Distress quest rules select the detailed route when a type supplies a hail, then enter the same complete, looping offer menu as local fleet quests; legacy cases without a hail retain the compact route. Catch.Release supplies the stranded fleet, dead-engine scavenger, and Hegemony Follower events through `FleetQuest`.
...
| File | Responsibility |
|---|---|
| `FleetQuest.java` | A `FishJob` attached to a fleet. Every construction path requires tutorial completion. Offers keep the source fleet from avoiding the player; accepted jobs replace it with a mission-owned copy. It renders saved case details into type-authored dialogue and exposes shared offer, counteroffer, and follow-up triggers so optional questions, alternate clients, and multi-stage decisions return to complete menus. Alternate-client selection preserves the ask while replacing the reward and all later contact, hand-in, intel, and thanks copy. Rewards come from `QuestRewards` at the type's multiplier, and deadlines are sized from the asks' nearest satisfiable water. Multi-stage hand-ins can pin the next request to the delivered species and reroll its reward before presenting an accept-or-decline follow-up. Hand-in uses the shared picker, reports `Received:` receipts, then returns the fleet to its source and despawns it after the final round. Distress variants rely on distress intel instead of a second marker. |
