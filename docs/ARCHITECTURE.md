...
| `HauntMineEntityPlugin.java` | The False Dawn's blinking mine, three tempers by colour: red bursts and shoves, blue interdicts and drags, yellow implodes with an inward distortion ripple and a damage-free pull. Arms after a beat, blinks faster as the fleet closes, triggers by proximity or by harpoon strike (`detonate` - the full show from range, but the shove, interdict and pull only land on a fleet inside the effect radius) and fades itself once its effect is spent |
