...
24. `sweepPondClaims()` — one walk, taking the mission marker off every rupture no errand is holding
...
| File | What it does |
|---|---|
| `FishEntityPlugin.java` | The swimming mote: motion archetypes, diving, held/stunned states, glow, and the exact source rupture retained for catch provenance. Pond motes let the terrain draw that glow inside its stencil; pondless motes draw themselves. Its `init` is where a legendary counts as sighted - every spawn path lands there - and a `phantom` mote (a haunt's decoy) skips that note, fails `isAvailable`, and shrugs off blasts, so no harpoon, drone or beam can touch it. It also carries the legendary defence behaviours: satellite orbit around a shield anchor with tether vfx, the flung dash and the moray's surviving travel dash, the shield bubble and deflection flash, the invert-blend reveal halo, the shell-game decoy state (steered by `QuorumShellGame`, fading when its real anchor is gone), and the Lantern Jack's mote-eating - all delegated to `LegendaryShields` |
...
| `HauntMineEntityPlugin.java` | The False Dawn's blinking mine, three tempers by colour: red bursts and shoves, blue interdicts and drags, yellow implodes with an inward distortion ripple and a damage-free pull. Arms after a beat, blinks faster as the fleet closes, triggers by proximity and fades itself once its effect is spent |
...
| `FishRouteSaveDialog.java` | Small centred dialog over the map, in the sidebar's visual language: name and purpose fields with ghost text, save and close. Pure UI - the host turns the confirmed pair into a `FishRouteIntel` |
...
| `MinefieldModule.java` / `entities/HauntMineEntityPlugin.java` | The False Dawn's minefield: waves of blinking mines seeded across the player's course, blink quickening as the fleet closes. Red bursts and shoves the fleet away, blue delivers the interdiction pulse with a short dragging slow, yellow implodes - an inward GraphicsLib ripple through `CampaignDistortionRenderer` and a damage-free pull toward where it was. Nothing harms the hull, and every mine is hard-removed on cleanup |
...
| `SlipDashModule.java` | The moray's escape: a straight surge away from the fleet that lays a live in-system slipstream along its path (`Terrain.SLIPSTREAM`, segments ordered so the current runs toward the fish), standing for a few seconds so a rider lands near the same water, then fading. The surfaced fish wears an inverted halo for a while so it can be found again |
| `QuorumShellGame.java` | The Quorum's endgame, session-transient and rebuilt from the decoys' persisted anchors on load: with the splinter escort gone, the school divides into three drifting bodies that trade places - two empty, one real, and the real one never moves during a split. A harpoon or blast into an empty body pops it before the shield or blast paths can misread its spec, and the school deals a fresh one from the real body's position |
| `LonglinerDecoy.java` | The Longliner's residency as a boat: a full Fisherman fleet through the same fittings as the real ones - identity, interactions, icon, coherence presence - flagged so `FishermanSpawner`'s reconciliation never books it. The one test it fails is the player's own breach lamp on its position: lit, the boat despawns and the running mote surfaces in its place, and the disguise stays blown until relocation. Never affected by any boat's own lamps |
...
| `FishRouteIntel.java` | A plotted route saved as a standing intel entry under its own **Fishing routes** category, with the player's name and purpose from the save dialog. Added to the manager directly, so it exists the moment the save confirms. Keeps its own copy of the stops - the live `FishRoute` keeps being replotted and cleared. The minigame's landed-catch hook bumps a per-species caught count on every tracking entry and sends the shared delayed update, so a needed catch notifies through the ordinary intel-update toast. The open panel shows the species strip on the shared icon stage, the quoted purpose, then each stop as its own bulleted block: every fish with its caught count and whether it is rupture-only, lamp-only or either, in rarity colours. Below that, which of the route's species still have live asks (jobs, errands, marks - read live from the planner's suggestion walk), and two buttons: re-plot the saved stops onto the hyperspace map, or stop tracking and remove the entry. Saved routes also draw their fish badges and arrow chains on the hyperspace map like the plotted route (through the overlay and filter script), and the entry supplies its own arrow chain to the intel screen's map |
...
| `harpoon/entities/HarpoonEntityPlugin.java` | The whole cast: flight, strike, hauling, catch, return, rope rendering. Pond and breach-lamp targets share one acquisition path that normalizes both to an ordinary fish mote before the common hold/shove state; a pond mote hands its exact parent rupture to the minigame while remaining the separate catch target, so harpooned specimens retain the same source proof as drone catches. The confirmed player mote strike asks `QuorumShellGame` before anything - an empty shell-game body pops and returns the line, never reaching the shield or blast paths - then `LegendaryShields` - a shielded legendary deflects the throw (no hook, no Retrieval Head refund) and popping the Longliner's shield spends the explosive head on the shield instead of the fish - then refunds one capped charge when Retrieval Head is fitted, and reports once before the ordinary/explosive split. A player's fitted explosive head gets a layered, irregular red warning pulse; its glow is a private filename-loaded sprite whose mutable render state is reset after every draw, never the shared campaign-entity sprite. Fleet collision eligibility excludes only the Fisherman, so normal and explosive shots pass through his boat and can hit something beyond it. An explosive impact records the mote's species name or struck fleet name before consuming the head - and an unshielded legendary never dies to it: it dives on the spot and resurfaces far away, the chase intact. An NPC-owned line skips the minigame and always lands |
...
(the Quorum's splinter) never spawn from ranges, take no job asks and no cap slot, and sit outside
the hundred.
...
charge-and-swallow shield, the moray's flung motes and rideable slip-dash, the False
