...
| `AberrationSource.java` | The registry: every kind of source as a row — label, tags (vanilla and foreign), light-year reach, weight, in-system reach, and whether it waits to be surveyed. Adding a source is a line; adding another mod's version of one is a tag |
