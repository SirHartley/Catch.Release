...
| File | Responsibility |
|---|---|
| `Tackle.java` | Defines modules, rig compatibility, optional icons, modifiers, and stock state. `BREACH_COUPLER` enables lamp openings for drones; `RETRIEVAL_HEAD` refunds a capped harpoon charge after a confirmed mote hit. |
...
| `rod/entities/FishingDroneEntityPlugin.java` | Drone launch, orbit, target selection, chase, catch, and return. Target locks sound only after launch and held catches are unavailable to every other rig. |
...
| `memory/upgrades/UpgradeStat.java` | One upgrade row: base, flat/multiplier progression, category, optional icon, and current value. |
...
| `helper/loading/UpgradeStatLoader.java` | Cached `UpgradeData.csv` loader with old and new column-name aliases and optional icons. |
