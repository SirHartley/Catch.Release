...
| `HarpoonPatrolResponse.java` | Sends one patrol at a time after the player. The infraction belongs to the space, not to a flag — but the collector is the offended faction itself or a third party that actually favours it (relationship at least favorable), and the Church and Path never carry water for the fishing trade: their patrols only come about their own holed fleets |
