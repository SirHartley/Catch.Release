...
| `FishRouteIntel.java` | A plotted route saved as a standing intel entry under its own **Fishing routes** category, with the player's name and purpose from the save dialog. Keeps its own copy of the stops - the live `FishRoute` keeps being replotted and cleared. The minigame's landed-catch hook bumps a per-species landed count on every tracking entry and sends the shared delayed update, so a needed catch notifies through the ordinary intel-update toast. The open panel shows the species strip on the shared icon stage, the quoted purpose, the stop list with landed counts in rarity colours, which of the route's species still have live asks (jobs, errands, marks - read live from the planner's suggestion walk), and two buttons: re-plot the saved stops onto the hyperspace map, or stop tracking and remove the entry |
