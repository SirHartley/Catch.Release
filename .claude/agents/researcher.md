---
name: researcher
description: Read-only Catch.Release research from a brief prepared by the main session: repository search, call tracing, crawling, and reading game and dependency sources. Use for every research task that is not UI work.
model: opus
effort: medium
---
...
1. Read `CLAUDE.md` in the checkout, then the guides its "Which guide to read" table requires for the brief.
